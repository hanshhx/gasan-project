...
export default function LocationMap() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans tracking-tight overflow-x-hidden selection:bg-[#E60012] selection:text-white break-keep">
      ...
      {/* 2. 타이틀 영역 */}
      <section className="relative pt-32 md:pt-40 pb-16 md:pb-20 bg-[#00152e] flex flex-col items-center justify-center overflow-hidden px-4">
        <div className="absolute inset-0 bg-[url('/images/building-1.jpg')] bg-cover bg-center opacity-20 mix-blend-overlay"></div>
        <div className="relative z-10 text-center reveal opacity-0 translate-y-12 transition-all duration-1000 ease-out">
          <span className="text-[#FFD700] font-black tracking-widest text-xs md:text-sm mb-3 md:mb-4 block uppercase">Premium Location</span>
          <h2 className="text-4xl md:text-6xl font-black text-white mb-4 md:mb-6 tracking-tight">광역위치도</h2>
          <div className="w-12 md:w-16 h-1 md:h-1.5 bg-[#E60012] mx-auto rounded-full mb-4 md:mb-6"></div>
          <p className="text-base md:text-xl text-slate-300 font-medium">성공 비즈니스를 위한 최적의 교통 네트워크</p>
        </div>
      </section>
      ...
      <main className="py-16 md:py-24 bg-slate-50">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 flex flex-col items-center">
          ...
          {/* 광역위치도 이미지 영역 */}
          <div className="w-full flex flex-col items-center gap-8 md:gap-12 mb-16 md:mb-24 reveal opacity-0 translate-y-12 transition-all duration-1000 delay-200 ease-out">
            <div className="w-full rounded-[1.5rem] md:rounded-[2rem] overflow-hidden shadow-md md:shadow-[0_20px_50px_rgba(0,0,0,0.08)] bg-white transform md:hover:scale-[1.01] transition-transform duration-500">
              <img src="/images/광역위치도-1.png" alt="가산디지털단지역 더블역세권" className="w-full h-auto object-contain" />
            </div>
            <div className="w-full rounded-[1.5rem] md:rounded-[2rem] overflow-hidden shadow-md md:shadow-[0_20px_50px_rgba(0,0,0,0.08)] bg-white transform md:hover:scale-[1.01] transition-transform duration-500">
              <img src="/images/광역위치도-2.png" alt="입지 및 인프라" className="w-full h-auto object-contain" />
            </div>
          </div>

          {/* ⭐️ 새롭게 디자인된 프리미엄 3단 카드 영역 ⭐️ */}
          <div className="w-full mb-8 md:mb-12">
            <div className="text-center mb-12 md:mb-16 reveal opacity-0 translate-y-12 transition-all duration-1000 ease-out">
              <h3 className="text-2xl sm:text-3xl md:text-4xl font-black text-[#002855] mb-3 md:mb-4">가산 3차 SK V1 center만의 <br className="sm:hidden"/><span className="text-[#E60012]">압도적 입지 가치</span></h3>
              <p className="text-slate-500 font-medium text-sm md:text-lg">기업의 현재와 미래를 완성하는 완벽한 비즈니스 환경</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8 reveal opacity-0 translate-y-12 transition-all duration-1000 delay-200 ease-out">
              ...
              <div className="bg-white rounded-[1.5rem] md:rounded-[2rem] p-8 md:p-10 shadow-md md:shadow-[0_10px_30px_rgba(0,0,0,0.04)] border border-slate-100 md:hover:shadow-[0_20px_40px_rgba(0,0,0,0.08)] md:hover:-translate-y-2 transition-all duration-300 flex flex-col relative overflow-hidden group">
                <div className="absolute top-0 left-0 w-full h-1.5 bg-gradient-to-r from-[#002855] to-blue-400 transform origin-left scale-x-0 md:group-hover:scale-x-100 transition-transform duration-500"></div>
                ...
                <h4 className="text-xl md:text-2xl font-black text-slate-800 mb-3 md:mb-4">최적의 산업 생태계</h4>
                <p className="text-slate-600 leading-relaxed text-sm md:text-[16px]">
                  서울 금천구 가산디지털단지 핵심 업무축에 위치합니다.<br/><br/>
                  서울 서남권 최대 규모의 산업·업무 집적지로, IT·제조·연구개발·벤처기업이 밀집해 <strong className="text-slate-900 font-black">기업 활동에 최적화된 산업 생태계</strong>를 형성하고 있습니다.
                </p>
              </div>

              {/* 카드 2: 쾌속 교통망 */}
              <div className="bg-white rounded-[1.5rem] md:rounded-[2rem] p-8 md:p-10 shadow-md md:shadow-[0_10px_30px_rgba(0,0,0,0.04)] border border-slate-100 md:hover:shadow-[0_20px_40px_rgba(0,0,0,0.08)] md:hover:-translate-y-2 transition-all duration-300 flex flex-col relative overflow-hidden group">
                <div className="absolute top-0 left-0 w-full h-1.5 bg-gradient-to-r from-[#E60012] to-red-400 transform origin-left scale-x-0 md:group-hover:scale-x-100 transition-transform duration-500"></div>
                <div className="w-14 h-14 md:w-16 md:h-16 bg-red-50 text-red-600 rounded-2xl flex items-center justify-center text-2xl md:text-3xl mb-6 md:mb-8">🚇</div>
                <h4 className="text-xl md:text-2xl font-black text-slate-800 mb-3 md:mb-4">쾌속 광역 교통망</h4>
                <p className="text-slate-600 leading-relaxed text-sm md:text-[16px]">
                  지하철 <strong className="text-[#E60012] font-black">1호선·7호선 가산디지털단지역</strong>을 중심으로 한 뛰어난 대중교통 접근성을 갖추고 있습니다.<br/><br/>
                  또한 남부순환로, 서부간선도로, 강변북로 등 주요 도로망과 연계돼 <strong className="text-slate-900 font-black">서울 도심과 수도권 전역으로의 이동이 편리</strong>합니다.
                </p>
              </div>

              {/* 카드 3: 업무 인프라 */}
              <div className="bg-white rounded-[1.5rem] md:rounded-[2rem] p-8 md:p-10 shadow-md md:shadow-[0_10px_30px_rgba(0,0,0,0.04)] border border-slate-100 md:hover:shadow-[0_20px_40px_rgba(0,0,0,0.08)] md:hover:-translate-y-2 transition-all duration-300 flex flex-col relative overflow-hidden group">
                <div className="absolute top-0 left-0 w-full h-1.5 bg-gradient-to-r from-[#FFD700] to-yellow-400 transform origin-left scale-x-0 md:group-hover:scale-x-100 transition-transform duration-500"></div>
                <div className="w-14 h-14 md:w-16 md:h-16 bg-yellow-50 text-yellow-600 rounded-2xl flex items-center justify-center text-2xl md:text-3xl mb-6 md:mb-8">☕</div>
                <h4 className="text-xl md:text-2xl font-black text-slate-800 mb-3 md:mb-4">완성도 높은 인프라</h4>
                <p className="text-slate-600 leading-relaxed text-sm md:text-[16px]">
                  주변에 수준 높은 업무시설과 상업시설, 그리고 다양한 편의시설이 함께 조성되어 있습니다.<br/><br/>
                  직원 출퇴근은 물론 기업 운영과 일상 편의까지 모두 충족할 수 있는 <strong className="text-slate-900 font-black">완성도 높은 업무 환경</strong>을 제공합니다.
                </p>
              </div>

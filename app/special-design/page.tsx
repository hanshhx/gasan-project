  ...
  return (
    <div className="min-h-screen bg-[#F7F5F2] text-black font-sans tracking-tight overflow-x-hidden selection:bg-[#9A000C] selection:text-white break-keep cursor-none">
      ...
      <section className="relative h-[45vh] min-h-[350px] md:h-[50vh] flex items-center justify-center overflow-hidden bg-gradient-to-br from-[#FFF9F5] via-[#FFF5EE] to-[#FFE8D8]">
        ...
        <div className="absolute top-10 right-20 w-[400px] h-[400px] bg-[#9A000C]/[0.02] rounded-full blur-[100px] pointer-events-none animate-[float_10s_ease_infinite]"/>
        <div className="relative z-10 text-center px-4">
          <span className="inline-block px-5 py-2 bg-transparent border border-[#9A000C]/20 text-[#9A000C] text-[10px] md:text-xs font-medium tracking-[0.4em] uppercase mb-6 rounded-none ag-slide-up">Specialized Design</span>
          <h2 className="text-4xl sm:text-5xl md:text-7xl font-serif text-gray-900 mb-6 tracking-tight ag-slide-up" style={{animationDelay:"0.15s"}}>특화설계</h2>
          <div className="w-16 h-[1px] bg-gray-300 mx-auto ag-slide-up" style={{animationDelay:"0.3s"}}/>
          <p className="text-base md:text-xl text-gray-500 font-light mt-7 max-w-2xl mx-auto leading-relaxed ag-slide-up tracking-wide" style={{animationDelay:"0.4s"}}>가산 3차 SK V1 center만의 압도적인 디테일<br className="hidden sm:block"/>완벽한 공간을 설계합니다</p>
        </div>
      </section>

      <main className="py-24 md:py-40">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 flex flex-col items-center gap-24 md:gap-32">
          {[
            {tag:"Detail 01",title:<>프라임 오피스의 <br className="sm:hidden"/><span className="italic font-light text-gray-400">혁신적 공간</span></>,img:"design-1.png",bg:"bg-transparent"},
            {tag:"Detail 02",title:<>완벽한 통제, <br className="sm:hidden"/><span className="italic font-light text-gray-400">압도적인 이동 속도</span></>,img:"design-2.png",bg:"bg-gray-900"},
            {tag:"Detail 03",title:<>임직원의 <br className="sm:hidden"/><span className="italic font-light text-gray-400">자부심이 되는 부대시설</span></>,img:"design-3.png",bg:"bg-transparent"}
          ].map((section,idx)=>(
            <div key={idx} className="w-full flex flex-col items-center ag-reveal" style={{transitionDelay:`${idx*0.2}s`}}>
              <div className="text-center mb-10 md:mb-14">
                <span className="inline-block text-[10px] text-gray-400 font-serif italic uppercase tracking-[0.3em] mb-4">{section.tag}</span>
                <h3 className="text-3xl md:text-4xl font-serif text-gray-900">{section.title}</h3>
              </div>
              <div className={`w-full overflow-hidden border ${section.bg==="bg-gray-900"?"border-gray-900":"border-gray-200"} group transition-all duration-700 ${section.bg}`}>
                {/* [수정: 디자인 12번 - 사진 등장 시 마스크 리빌 효과] */}
                <div className="relative w-full">
                  <div className={`absolute inset-0 z-20 mask-reveal-anim ${section.bg==="bg-gray-900"?"bg-black":"bg-gray-100"}`} />
                  <img src={`/images/${section.img}`} alt={`${section.tag}`} className={`w-full h-auto object-contain group-hover:scale-[1.02] transition-transform duration-[2000ms] ${section.bg==="bg-gray-900"?"opacity-90 group-hover:opacity-100 mix-blend-luminosity hover:mix-blend-normal":""}`}/>
                </div>
              </div>
            </div>
          ))}
          ...
          {/* [수정: 딥 버건디 배경 배너] */}
          <div className="ag-reveal w-full bg-[#4A0A0A] rounded-none p-12 sm:p-20 md:p-32 text-center relative overflow-hidden">
            <div className="absolute inset-0 bg-[url('/images/building-1.png')] bg-cover bg-center opacity-[0.05] mix-blend-overlay"/>
            <div className="absolute inset-0 overflow-hidden pointer-events-none opacity-5"><div className="whitespace-nowrap animate-[marquee_40s_linear_infinite] text-[10rem] font-black text-white leading-none mt-10">SK V1 CENTER · PREMIUM · SK V1 CENTER ·&nbsp;</div></div>
            <div className="relative z-10">
              <h3 className="text-3xl sm:text-4xl md:text-5xl font-serif text-white leading-tight mb-8">기업의 가치를 높이는 <span className="italic font-light text-white/60 block sm:inline mt-4 sm:mt-0">가산 3차 SK V1</span></h3>
              <div className="w-16 h-[1px] bg-white/20 mx-auto mb-8"/>
              <p className="text-sm md:text-base text-white/50 font-light max-w-2xl mx-auto leading-loose tracking-widest px-2">디테일이 성공의 크기를 결정합니다. 실입주 기업의 다양한 니즈를 완벽하게 충족시키는 최상의 업무 환경을 직접 확인해 보세요.</p>
            </div>
          </div>
        </div>
      </main>
      ...
      <style jsx global>{`
        ...
        @keyframes gentleBounce{0%,100%{transform:translateY(0)}50%{transform:translateY(-5px)}}@keyframes marquee{0%{transform:translateX(0)}100%{transform:translateX(-50%)}}
        ...
        @keyframes maskReveal{0%{transform:translateX(0)}100%{transform:translateX(100%)}}
        .is-revealed .mask-reveal-anim { animation: maskReveal 1.5s cubic-bezier(0.77, 0, 0.175, 1) forwards; }
